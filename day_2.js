...
const opcodeReader = (intcode, position = 0) => {
    ...
    return opcodeReader([...intcode], position+4);
};

const phraseFinder = (intcode) => {
    for (let i=0; i <= 99; i++) {
        intcode[1] = i;
        for (let u=0; u <= 99; u++) {
            intcode[2] = u;
            const phraseOutput = opcodeReader([...intcode]);
            if (phraseOutput === 19690720) return 100 * i + u;
        }
    }
};
